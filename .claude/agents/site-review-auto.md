...
| 15     | **design-standardizer mandatory.** Phase 2 design sweep MUST spawn a Task sub-agent (Sonnet, default tools) referencing `.claude/agents/design-standardizer.md`. Do not skip even if Phase 1 finds zero violations. Record Task run result in state; on sub-agent failure, log as `theme: "design"` / `status: "sub_agent_error"` finding. |
...
- **design-standardizer.md:** NEW agent, invoked as Task by site-review-auto Phase 2. design-standardizer is also callable on-demand by users or design-guardian.
...
3. ✓ Phase 2: All 4 Task sub-agents (SSOT, page-team, design-standardizer, recheck) returned results; findings merged.
